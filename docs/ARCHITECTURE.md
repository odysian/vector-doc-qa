...
- **Terraform ops auth (OIDC/WIF):** GitHub Actions uses Google Workload Identity Federation (`github-actions-pool` + `github-actions-provider`) with `roles/iam.workloadIdentityUser` bound to the Terraform ops service account.
- **Deploy auth (IAP tunnel + ephemeral key injection):** The deploy workflow authenticates via the same WIF pool/provider, but impersonates a separate `quaero-github-deploy-sa` service account (separation of duties). `gcloud compute ssh/scp --tunnel-through-iap` replaces direct port-22 SSH. Connections use ephemeral SSH keys injected at VM instance metadata for user `odys`. Required GitHub secrets: `GCP_WIF_PROVIDER`, `GCP_DEPLOY_SA_EMAIL`, `GCP_VM_NAME`, `GCP_PROJECT_ID`, `GCP_VM_ZONE`. Removed secrets: `GCP_VM_SSH_KEY`, `GCP_VM_HOST`. See ADR-009.
...
- **CI trigger policy:** Both `Backend CI` and `Frontend CI` run on every `pull_request` event using a three-job gate pattern: a lightweight `changes` job always runs a `git diff` to detect touched paths; a `backend-tests` / `frontend-tests` job runs the full suite only when relevant files changed (`backend/**` or `frontend/**` respectively); a `backend-verify` / `frontend-verify` gate job always runs and exits 0 when tests passed or were skipped, so branch protection always receives a result. No `push:` trigger on either workflow — bare branch pushes without a PR do not run CI by design. Both workflows expose a `workflow_call:` trigger for deploy pipeline use (always runs full suite when called). Frontend CI steps: `npm ci` → `tsc --noEmit` → `npm run lint` → `vitest run` → `next build` → `npm audit --audit-level=high`.
...
- **Control-plane guardrail:** `main` branch protection should require status checks `Backend CI / backend-verify` and `Frontend CI / frontend-verify`.
