...
- **Deploy auth (IAP tunnel + OS Login):** The deploy workflow authenticates via the same WIF pool/provider, but impersonates a separate `quaero-github-deploy-sa` service account (separation of duties). `gcloud compute ssh/scp --tunnel-through-iap` replaces direct port-22 SSH; OS Login manages ephemeral key injection. Required GitHub secrets: `GCP_WIF_PROVIDER`, `GCP_DEPLOY_SA_EMAIL`, `GCP_VM_NAME`, `GCP_PROJECT_ID`, `GCP_VM_ZONE`. Removed secrets: `GCP_VM_SSH_KEY`, `GCP_VM_HOST`. See ADR-009.
