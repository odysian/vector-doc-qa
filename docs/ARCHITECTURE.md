...
- **CI trigger policy:** `Backend CI` runs on every PR and direct push to non-`main` branches when `backend/**` or `.github/workflows/backend-test.yml` changes. A shared commit SHA concurrency group deduplicates the `push` + `pull_request:synchronize` double-fire on PR branch commits so exactly one run executes per commit.
