...
- **Bootstrap model:** VM startup script is a stable launcher that provisions `quaero-reconcile.service`/timer; runtime host reconciliation (Docker, NGINX, Certbot, Ops Agent, `/opt/quaero` directories) is versioned via external reconcile artifact with metadata hash pin/verification (`reconcile_release_id` + `reconcile_sha256`) from a dedicated read-only runtime reconcile bucket. Production `backend.env` is pushed by deploy workflow from GitHub secret `BACKEND_ENV_B64`.
- **Cutover governance:** Production rollout identity is the tuple (`vm_image` as image version + `reconcile_release_id`) plus deterministic pins (`infra_commit_sha` + `reconcile_sha256`) to guarantee reproducible rollback from the intended infra commit; checkpoint, health gates, and rollback drill evidence are tracked in `docs/GCP_RUNBOOK.md` (decision rationale: `docs/adr/008-vm-golden-image-cutover-and-tuple-rollback.md`).
