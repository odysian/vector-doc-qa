...
`alembic/env.py` includes several guards to make `alembic check` and `alembic revision --autogenerate` deterministic for the `quaero` schema:
...
**process_revision_directives guardrails:** Runs during `alembic revision --autogenerate` (not during `alembic check`) and blocks:
- `DROP TABLE` on non-app objects
- Any operation targeting a non-`quaero` schema
- FK schema-qualification churn (remove+add pairs that only differ in schema prefix)

**Override path:** Set `ALEMBIC_UNSAFE_AUTOGEN=1` to bypass guardrails for intentional changes that would normally be blocked.
...
- Never commit a migration that contains `drop_table` without explicit approval
- Never commit a migration that only contains FK add/remove pairs (schema churn)
